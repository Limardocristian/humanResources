...
const controller = {
  ...
  login: function (req, res) { 
    return res.send( Document.findAll({ where: { id_tipo_document: req.body.id_tipo_document } }) );
    const errors = validationResult(req);
    // const {email, password} = req.body
    // const user ={email:email}
    // const accesToken = generateAccessToken(user);
  ...
  generateAccessToken: function (document) { 
    return jwt.sign(document,process.env.DB_HOST,{expiresIn:'5m'});
  },

  register: function (req, res) { 
    const errors = validationResult(req);
    if (errors.isEmpty()) { 
    req.body.id_tipo_document = bcrypt.hashSync(req.body.id_tipo_document, 10);
    Document.create({
          id_tipo_document: req.body.id_tipo_document,            
      })
      .then(() => {
        return res.send('el documento se creo correctamente');
      })
    } else {
      return res.send(errors);
    }
